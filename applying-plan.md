...
For more information on viewing successful or failed cluster deployments, see [Retrieving apply plan logs](/docs/hpc-spectrum-scale?topic=hpc-spectrum-scale-retrieve-apply-plan-logs).

## Next steps
{: #apply-plan-ui-next-steps}
{: ui}

After you've successfully applied a plan, you can begin [Accessing the GUI](/docs/hpc-spectrum-scale?topic=hpc-spectrum-scale-access-gui) to monitor the resources and status of the service for both the compute and storage clusters.
...
2. Copy and paste the [Apply a plan using {{site.data.keyword.bpshort}} Python API](/docs/hpc-spectrum-scale?topic=hpc-spectrum-scale-applying-plan&interface=api#example-request-apply) example request to your Python file.
