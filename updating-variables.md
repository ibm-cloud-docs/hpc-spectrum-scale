...
3. Copy and paste the [`config.json` template file](/docs/hpc-spectrum-scale?topic=hpc-spectrum-scale-update-variables&interface=api#template-file) to a JSON file, for example `config.json`.
...
5. Make sure to update the required parameters in the `config.json` file. For more information, see [Deployment values](/docs/hpc-spectrum-scale?topic=hpc-spectrum-scale-deployment-values).
